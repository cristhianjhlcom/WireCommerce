<div class="space-y-4">
  ...
  <flux:text class="mt-2">{{ __('Manage tags of the system.') }}</flux:text>
  <flux:separator />
  ...
  <div class="flex flex-col space-y-4">
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
      ...
      <flux:button.group>
        ...
        <flux:button href="{{ route('admin.tags.create') }}" icon="plus">
          {{ __('Add Tag') }}
        </flux:button>
      </flux:button.group>
    </div>
          ...
          <flux:table.row key="{{ $tag->id }}">
            <flux:table.cell class="flex items-center gap-3">
              <div class="flex flex-col gap-y-2">
                <flux:text>{{ $tag->icon }} {{ $tag->name }}</flux:text>
                <flux:text>{{ $tag->slug }}</flux:text>
              </div>
            </flux:table.cell>
            <flux:table.cell>{{ $tag->createdAtHuman() }}</flux:table.cell>
            <flux:table.cell>{{ $tag->updatedAtHuman() }}</flux:table.cell>
            <flux:table.cell>
              <flux:dropdown align="end" position="bottom">
                <flux:button icon="ellipsis-horizontal" variant="ghost"></flux:button>
                <flux:menu>
                  <flux:menu.item href="{{ route('admin.tags.edit', $tag) }}" icon="pencil">
                    {{ __('Edit') }}
                  </flux:menu.item>
                  <flux:menu.item
                    icon="trash"
                    variant="danger"
                    wire:click="delete({{ $tag }})"
